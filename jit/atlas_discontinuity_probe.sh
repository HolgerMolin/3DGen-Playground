#!/bin/bash
# Atlas discontinuity probe — visualize where in the Gaussian atlas x0-prediction MSE is
...
PY_ARGS=(
    jit/atlas_discontinuity_probe.py
    --model "$MODEL"
    --weights "$WEIGHTS"
    --obj_list "$OBJ_LIST"
...
echo "Running Atlas discontinuity probe (foreground): model=$MODEL weights=$WEIGHTS t=$T_VALUES N=$NUM_SAMPLES bs=$BATCH_SIZE"
exec python "${PY_ARGS[@]}" "$@"
